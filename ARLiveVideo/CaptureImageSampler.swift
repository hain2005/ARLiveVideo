...
class CapturedImageSampler {
    ...
    init(frame: ARFrame) throws {
        ...
        print("rgb size \(rgb.hashValue)")
        print("rgb size \(rgbSize)")
    }
    ...
    static func inspect(pixelBuffer: CVPixelBuffer) {
        ...
        // Access pixel information as needed
        // Example: Extract the color of the pixel at x=10, y=10
        let pixel = baseAddress!.advanced(by: (bytesPerRow * 10) + (4 * 10))
...
struct BufferDimension {
    ...
    func convertImageToRedArray(image: UIImage) -> [[UInt8]]? {
        ...
        var redArray = [[UInt8]](repeating: [UInt8](repeating: 0, count: width), count: height)
